...
A reminder in the Status Bar appears to indicate that to exit/stop the command and get back to normal text insertion/deletion you can exit the `multiMode` by hitting the <kbd>Return</kbd> (unfortunately <kbd>Escape</kbd> will **not work**).  That Status Bar reminder will hide when you do exit the command successfully and reappear the next time you invoke a MultiMode command.  

You can also exit `multiMode` by clicking on the `StatusBar` notice or by invoking the command `Jump-Select: Abort MultiMode` (`jump-and-select.abortMultiMode`) either from a keybinding you set up or through the Command Palette.  
